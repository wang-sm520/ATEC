...
class B2Locomotion:
    ...
    GAIT_CYCLE_TIME = 0.5
    # New B2 policy is 45-dim (= demo B2 policy MINUS the 2 gait-phase obs terms);
    # action scale and obs scalings are unchanged. Flip to False so act() builds
    # the 45-dim obs. The phase machinery (GAIT_CYCLE_TIME, _phase_step, reset(),
    # PHASE_INSERT_AFTER_COMMAND) is left inert so the flag stays toggleable.
