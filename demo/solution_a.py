...
class AlgSolution:
    ...
    def predicts(self, obs, current_score):
        ...
        # Integrate yaw rate to estimate heading drift, then P-control toward
        # HEADING_TARGET=0. Output goes into cmd[2] just like the env did during training.
        #
        # --- 改进思路1: 去倾斜投影 (de-tilt) ---
        # 旧逻辑(体系 z 轴角速度直接积分;粗糙地面/坡上躯干 pitch/roll 时,体系 z ≠ 世界 z,
        #        积分的是"歪轴", 误差单向累积 → 航向漂移):
        #   self.heading_est += float(base_ang_vel[2].item()) * self.STEP_DT
        # 新逻辑: projected_gravity 指向世界"下", 取反归一化 = 世界竖直("上")轴在体坐标系下的方向;
        #        体系角速度在该轴上的投影 = 真正的世界 yaw 速率, 不再受 pitch/roll 影响。
        #        机器人直立时 up_body≈[0,0,1], 退化为原来的 ang_vel[2], 行为不变。
